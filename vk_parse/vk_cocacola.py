...
class RuDate(ComparableMixin):
    def __init__(self, s):
        ...
        self.dat = "%02d.%02d.%04d" % (self.d, self.m, self.y)
...
def items_list_wall(n, topic, topic_text, getvar, last=0, d=50):
    ...
    for i,a in enumerate(a_anchors):
        ...
        if dd >= RuDate(u'3 июн') and nn > last:  
    ...
    for a in a_all:
        if 'name' in a.attrMap.keys():
            if 'post' in a.attrMap['name']:
                ...
                tt = ai.text
                if nn > last:
                    items[nn] = ( ai.text, topic_text, topic, aa.text, aa['href'], at.text, ai.attrMap['href'] )
                    topics[topic]['lastN'] = nn
                    topics[topic]['count'] += 1
                    count += 1
                    dates[nn] = {'date':dd.get(), 'time':tt}
...
def print_items(ofile, title=False):
    ...
    if not add:
        ...
        if title:
            titles = (u'#',u'Дата комментария',u'Тема дискуссии',u'URL дискуссии',
                      u'Автор комментария',u'URL автора',u'Текст комментария',u'URL комментария',
    ...
    for k in sorted(items.keys()):
        ...
        #print k, "%s \n%s\n%s\n%s\n%s" % items[k]
        s = u"%d" % k
        s += u'|%s|%s|http://vk.com/%s|%s|http://vk.com%s|%s|http://vk.com%s| \n' % items[k]
...
def itertopics(picklefile, newpic=True):
    ...
    if newpic:
        ...
        topics['topic-16297716_28039770']['lastN'] = 107482#106434 #107482
        topics['topic-16297716_28039770']['offset'] = 20 * (457 - 1)
        topics['topic-16297716_28039770']['do'] = 1 
        topics['topic-16297716_28039770']['count'] = 0 
        topics['topic-16297716_28124405']['firstN'] = 105815
        topics['topic-16297716_28124405']['lastN'] = 107442#106393 #107442
        topics['topic-16297716_28124405']['offset'] = 20 * (12 - 1)
        topics['topic-16297716_28124405']['do'] = 1
        topics['topic-16297716_28124405']['count'] = 0
        topics['topic-16297716_22922336']['firstN'] = 105762
        topics['topic-16297716_22922336']['lastN'] = 107470#106435 #107470
        topics['topic-16297716_22922336']['offset'] = 20 * (334 - 1)
